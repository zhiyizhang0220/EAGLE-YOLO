...
default_hooks = dict(
    ...
    visualization=dict(
        draw=True, test_out_dir='test_save', type='DetVisualizationHook'))
default_scope = 'mmdet'
env_cfg = dict(
...
launcher = 'none'
load_form = None
load_from = 'work_dirs/yolox_tiny_8xb8-300e_visdrone/epoch_50.pth'
log_level = 'INFO'
log_processor = dict(by_epoch=True, type='LogProcessor', window_size=50)
max_epochs = 300
metainfo = dict(classes='maize')
model = dict(
    backbone=dict(
...
test_dataloader = dict(
    batch_size=32,
    dataset=dict(
        ann_file='test/annotations/test_fixed.json',
        backend_args=None,
...
test_evaluator = dict(
    ann_file='E:/Yolo/plus/test/annotations/test_fixed.json',
    backend_args=None,
    metric='bbox',
    type='CocoMetric')
test_pipeline = [
...
train_cfg = dict(max_epochs=300, type='EpochBasedTrainLoop', val_interval=100)
train_dataloader = dict(
    batch_size=16,
    dataset=dict(
        dataset=dict(
            ann_file='train/annotations/train_fixed.json',
            backend_args=None,
...
val_dataloader = dict(
    batch_size=16,
    dataset=dict(
        ann_file='val/annotations/val_fixed.json',
        backend_args=None,
    ...
    sampler=dict(shuffle=False, type='DefaultSampler'))
val_evaluator = dict(
    ann_file='E:/Yolo/plus/val/annotations/val_fixed.json',
    backend_args=None,
    metric='bbox',
    type='CocoMetric')
vis_backends = [
...
work_dir = './work_dirs\\yolox_tiny_8xb8-300e_visdrone'
